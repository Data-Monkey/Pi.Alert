...
def send (msg: noti_struc):
    """ sending the message to apprise """
    html = msg.html
    text = msg.text
    ...
    try:
        ...
        process = subprocess.Popen(["curl","-i","-X", "POST" ,"-H", "Content-Type:application/json",
                                    "-d", json.dumps(_json_payload), conf.APPRISE_HOST],
